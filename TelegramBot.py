...
# # ==========================================================
# # ⚠️ BLOCK MANUAL PHONE INPUT (keep Share button visible)
# # ==========================================================
...
#         reply_markup = ReplyKeyboardMarkup(keyboard, one_time_keyboard=False, resize_keyboard=True)
#         await update.message.reply_text(
#             "⚠️ Please use the *Share Phone Number* button below.\n"
...
# # # ==========================================================
# # # 🖼️ PASSPORT UPLOAD HANDLER (Plan ID + API save with retries & smaller JPEG)
# # # ==========================================================
# # async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
# #     chat_id = str(update.message.chat.id)
# #     state = USER_STATE.get(chat_id, {})
...
# #     photo = update.message.photo[-1]  # highest resolution; we’ll downscale
# #     photo_file = await photo.get_file()
...
# #     print("✅ Image prepared as JPEG (<=1600px)")

# #     # --- Fetch Plan ID (best-effort; won’t block saving) ---
# #     plan_id = "N/A"
# #     try:
# #         resp = await asyncio.to_thread(
# #             requests.get,
...
# #                 print(f"[WARN] No matching Plan ID found for {target_country} ({target_data})")
# #         else:
# #             print(f"[ERROR] Plan ID API returned {resp.status_code} → {resp.text}")
# #     except Exception as e:
# #         print(f"[ERROR] Could not fetch plan ID: {e}")
...
# #     # --- Upload image + phone to API ---
# #     try:
# #         # ✅ Extract only last 10 digits for backend
# #         phone_number_original = state.get("phone", "")
# #         phone_number_digits = ''.join(ch for ch in phone_number_original if ch.isdigit())
...
# #     keyboard = [[InlineKeyboardButton("💳 Buy Now", url=payu_link)]]
# #     await update.message.reply_text(
# #         f"✅ Image received and details processed!\n\n"
# #         f"📋 *Order Summary:*\n"
# #         f"🌍 Country: *{country}*\n"
# #         f"📦 Package: *{data_size}*\n"
# #         f"💰 Price: ₹{price}\n"
...
# # ==========================================================
# # 🖼️ PASSPORT UPLOAD HANDLER (with Plan ID + proper mobile + API save)
# # ==========================================================
# async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
#     chat_id = str(update.message.chat.id)
#     state = USER_STATE.get(chat_id, {})
...
#     # Save image locally
#     await photo_file.download_to_drive(file_path)
#     print(f"✅ Image prepared as JPEG (<=1600px)")
...
#     keyboard = [[InlineKeyboardButton("💳 Buy Now", url=payu_link)]]
#     await update.message.reply_text(
#         f"✅ Passport image and phone number saved successfully!\n\n"
#         f"📋 *Order Summary:*\n"
#         f"🌍 Country: *{country}*\n"
#         f"📦 Package: *{data_size}*\n"
#         f"💰 Price: ₹{price}\n"
...
# ==========================================================
# 🧭 CALLBACK HANDLER (only BACK option kept)
# ==========================================================
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    try:
        ...
        # ---- PACKAGE ----
            ...
            if not data_sizes or all("⚠️" in d for d in data_sizes):
                back_buttons = [
                    [InlineKeyboardButton("🔙 Back to Packages", callback_data=f"country|{state['country']}")]
                ]
                await query.edit_message_text(
                    f"⚠️ Sorry, no *{package_type}* plans are available for *{state['country']}*.\n"
